...
class Image {
	...
	function initialize($props = array())
	{
		...
		else if($this->width && !$this->height)
		{
			...
			$ratio = $this->width / $this->orig_width;
			$this->height = $this->orig_height * $ratio;
		}
		else if($this->height && !$this->width)
		{
			...
			$ratio = $this->height / $this->orig_height;
			$this->width = $this->orig_width * $ratio;
		}
	 ...
	 * @return	bool
	 */
	function resize($source_image,$new_image,$width=0,$height=0,$quality=90,$x='', $y='')
	{
		if($source_image)
		{
			...
			$config['quality']			= $quality?$quality:90;
		...
		if ($this->dynamic_output === FALSE)
		{
			if ($this->orig_width == $this->width AND $this->orig_height == $this->height)
			{
				if ($this->source_image != $this->new_image)
				{
					...
					{
						@chmod($this->full_dst_path, FILE_WRITEMODE);
					}
				}
